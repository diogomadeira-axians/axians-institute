    ...
    return (
        <footer className="py-16 space-y-4 bg-white">
            <div className="container mx-auto gap-y-6 flex flex-col-reverse items-center md:flex-row md:items-start">
                ...
                <ul className="mx-8 grid grid-cols-1 md:grid-cols-3 md:grid-rows-3 gap-x-6 gap-y-3 ">
                    {menus.map(eachMenu => {
                        return(
                            <li
                                className={`font-normal text-center text-brand-primary-dark min-w-20 row-span-1 md:text-start ${eachMenu.cssClass}`}
                                key={eachMenu.text}
                            >
